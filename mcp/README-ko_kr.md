...
여러 URL에서 메타데이터를 동시에 추출합니다.

| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| `urls` | `string[]` | O | 스크래핑할 URL 목록 |
| `concurrency` | `number` | X | 동시 요청 수 (기본값: 5, 최대: 20) |

### `detect_feeds`

웹 페이지에서 RSS와 Atom 피드 링크를 감지합니다.

| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| `url` | `string` | X | 피드를 감지할 URL |
| `html` | `string` | X | 피드를 감지할 HTML 문자열 |

### `check_robots`

웹 페이지의 robots 메타 태그 디렉티브와 인덱싱 상태를 확인합니다.

| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| `url` | `string` | X | robots 디렉티브를 확인할 URL |
| `html` | `string` | X | robots 디렉티브를 확인할 HTML 문자열 |

### `validate_metadata`

메타데이터 품질을 검증하고 SEO 점수 리포트(100점 만점)를 생성합니다.

| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| `url` | `string` | X | 메타데이터를 검증할 URL |
| `html` | `string` | X | 메타데이터를 검증할 HTML 문자열 |

### `extract_content`

웹 페이지에서 본문 텍스트를 추출합니다 (네비게이션, 광고, 사이드바 제거).

| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| `url` | `string` | X | 콘텐츠를 추출할 URL |
| `html` | `string` | X | 콘텐츠를 추출할 HTML 문자열 |
...
| **Favicons** | 모든 아이콘 링크 + `sizes`, `type` |
| **Feeds** | RSS 및 Atom 피드 링크 + `title` |
| **Robots** | Robots 메타 디렉티브 및 인덱싱 가능 여부 플래그 |
